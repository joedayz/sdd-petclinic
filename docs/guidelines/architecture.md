...
Single Maven module, **package-by-feature** under `pe.joedayz.petclinic` — each feature (e.g. `owner`, `pet`, `visit`, `vet`,`welcome` ) is its own package with two subpackages:
